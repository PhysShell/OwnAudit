...
"""Build a self-contained interactive HTML dashboard from the STS audit artifacts.
    ...
    python3 viz/build_dashboard.py [DATA_DIR]   ->  viz/sts-dashboard.html

Reads <DATA_DIR>/findings.json + <DATA_DIR>/health-report.md (the module pain table).
DATA_DIR is any audit-output folder (default: artifacts) — pass a positional path to
point at another run (e.g. sts_audit, or a compare dir). Emits one standalone HTML file
(Plotly inlined when viz/plotly.min.js
...
STS = os.path.join(ROOT, "artifacts")
...
<footer>Generated by <code>viz/build_dashboard.py</code> from <code>artifacts/</code> · raw findings clustered to %CLUSTERS% (high-confidence = ≥2 tools agree).</footer>
<script>
const D = %DATA%;
const D_ = D.dims;
