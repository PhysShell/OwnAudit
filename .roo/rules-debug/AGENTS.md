...
- `Run-Audit.ps1` splits across two repos: normalization is local (`aggregate/normalize.py`), while scoring/reporting still run from the external Own.NET worktree (`audit/aggregate/report.py`) — read the failing path before assuming which side broke.
