...
|---|---|---|
| Реализация CLI `ownnet` (Python-фасад) | **OwnAudit** | integration-репо; CLI-модули (`report/`, `fix/`, `arch/`) и mutating-поверхность (fix-arm) уже здесь; туда же лифтится `audit/` |
...
1. Пин Own.NET в OwnAudit для воспроизводимого `ownnet audit`: git-submodule или
   зафиксированный worktree-подход `Run-Audit.ps1` (сейчас — плавающий main)?
2. Бренд/имя бинаря: `own-audit` → `ownnet` rename или новый entry point рядом?
3. Дистрибуция Python-CLI: `uv tool` / `pipx` — определить целевой способ установки
   для агентских окружений.
4. Версионирование схем: `ownnet.finding.v1` — где держать changelog при эволюции
   (предложение: `Own.NET/spec/`, рядом с движком).
