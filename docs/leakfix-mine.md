...
`semantic`-тире), а не в «поймаем ли течь». Zig/Nim/Odin — отдельный appendix
(`docs/leakfix-langs-appendix.md`): рыночно-нишевая оценка + рамка **OwnSys** (Zig+Odin как
один allocator-aware чекер), query-паки в `signals.py`. Gleam пропускаем, пока не наберётся
actor/process-lifecycle (и это всё равно ниша **BEAM**, т.е. целиться надо в Elixir, а не Gleam).
