...
- **snapshot** графа — компактный, коммитимый: метрики на компонент + namespace-поверхность
  зависимостей (включая рёбра во внешние фреймворки — там и живёт «new SQL dependency») +
  множество циклов. Baseline-снапшот живёт в репо, как в фазе 2.
